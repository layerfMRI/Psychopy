...
# Declare stimulus and response parameters
    'nBlocks': 12,            # number of blocks in this session
    'condition': 'TapRight',
...
stimList = {
    'conditionList':['TapRight','TapFast','TapLeft','AlmostRight','ImagineRight','sensationOnly'],
    'moviePromptList': ['Tap along with the video.','Tap along with the video.','Tap along with the video.','Move (Right Hand) but do NOT touch fingers \nAlong with the video.','Imagine Tapping (Right Hand).','feel the hand'],
    'moviePrefixList': ['right','right','left','right_almost','right_imagine','right_sensation'], # filenames of movies
    'movieFrameRateList': [10, 30, 30, 10, 1,1], # frame rate of each movie (s)
    'movieNFrameList':[10, 10, 10, 10, 1,1], # nFrames in each movie (numbered from 0 to nFrames-1
    'promptFileList': ['TappingPrompts_Movie.txt','TappingPrompts_Movie.txt','TappingPrompts_Movie.txt','AlmostPrompts_Movie.txt','ImaginePrompts_Movie.txt','SensationPrompts_Movie.txt'] # Name of text file containing prompts 
}
# save parameters
if saveParams:
...
# ===== GET SCREEN RES ===== #
# ========================== #
#win = visual.Window(size=(1920, 1200), fullscr=False, screen=0, allowGUI=False, allowStencil=False,
#    monitor=u'7T_Scanner_UM', color=[0,0,0], colorSpace='rgb',
...
def CoolDown():
    ...
    message1.setText("Thank you very much! \n\nThat's the end of this experiment")
