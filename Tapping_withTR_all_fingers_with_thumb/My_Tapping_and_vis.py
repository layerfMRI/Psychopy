...
Instr_TAP5=visual.TextStim(win,text=Instr_TAP5,height=100,units='pix',name='intro', color='black',wrapWidth=600,pos=(0,0));
...
while numbtrials<numTrials:
    ...
    while numbtrigger<restDur:
        ...
        numbtrigger=numbtrigger+1
    numbtrigger=0
    while numbtrigger<visStimDur*2:
        for keys in event.getKeys(timeStamped=True):            #handle key presses each frame
                if keys[0]in ['escape','q']:
                    win.close()
                    core.quit()
        #stimBCent.draw();win.flip();
        Instr_TAP5.draw();win.flip();
        event.waitKeys(keyList=['t'])
